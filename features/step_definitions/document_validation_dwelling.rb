Then(/^I save additional details for Dwelling Binder$/) do
...
Then(/^I validate the Binder Document for Dwelling$/) do
  ...
  PDF::Reader.open(File.open(my_pdf, "rb")) do |reader|
    ...
    arr_first = [expected_name, expected_addr, expected_city, expected_email,
                 @insured_name, @insured_city, @insured_state, @insured_zipcode, @insured_email_address, @annual_premium]
...
Then(/^I save additional details for dwelling Policy Declaration$/) do
...
Then(/^I validate the Policy Declaration Document for dwelling$/) do
  ...
  PDF::Reader.open(File.open(my_pdf, "rb")) do |reader|
    ...
    arr_second = [@form_name_0, @form_date_0, @form_title_0, @form_name_1, @form_date_1, @form_title_1,
                  ...
                  @form_name_18, @form_date_18, @form_title_18]
    reader.pages.each do |page|
      if page.number == 1
        arr_first.each do |str|
          expect(page.text.include? str).to be_truthy, "String #{str} is not found in the pdf"
...
Then(/^I save additional details for Application Binder$/) do
  @insured_name =  @doc.xpath('.//InsuredInformation').css('Name')[1].text
  @insured_city = @doc.xpath('.//InsuredInformation').css('City').text
  @insured_state = @doc.xpath('.//InsuredInformation').css('State').text
  @insured_zipcode = @doc.xpath('.//InsuredInformation').css('ZipCode').text
  @insured_email_address = @doc.xpath('.//InsuredInformation').css('EmailAddress').text
  @website = @doc.xpath('.//AgencyInfo').css('WebSite').text
...
Then(/^I validate the Binder Document for Application$/) do
  ## Data from automation
  data_used = EDSL::PageObject.fixture_cache
  my_pdf = @document_location
  ...
  PDF::Reader.open(File.open(my_pdf, "rb")) do |reader|
    ...
    arr_first = [expected_name, expected_addr, expected_city, @insured_email_address,
                 ...
                 @quote_number, @prepared_by, @website]
...
Then(/^I save additional details for Dwelling Quote$/) do
...
Then(/^I validate the Binder Document for Quote$/) do
  data_used = EDSL::PageObject.fixture_cache
  my_pdf = @document_location
  ...
  PDF::Reader.open(File.open(my_pdf, "rb")) do |reader|
    ...
    arr_first = [expected_name, expected_addr, expected_city,
                 ...
                 @quote_number, @prepared_by, @semi_annual_installment_amount,
                 @quarterly_installment_amount, @monthly_installment_amount, @quote_Premium]
...
Then(/^I save additional details for dwelling ISO Location PPC Inquiry Form$/) do
  @policy_symbol =  @doc.xpath('.//PolicySymbol').text
  @policy_number =  @doc.xpath('.//PolicyNumber')[1].text
end
Then(/^I validate the ISO Location PPC Inquiry Form for dwelling$/) do
  ...
  PDF::Reader.open(File.open(my_pdf, "rb")) do |reader|
    ...
    arr_first = [@policy_symbol, @policy_number]
