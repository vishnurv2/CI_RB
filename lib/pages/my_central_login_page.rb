# frozen_string_literal: true

class MyCentralLoginPage < BasePage
  page_url "https://saturn.central-insurance.com/myCentral/Membership/mycentrallogin.aspx"

  # ------ Everything on this page has been VERIFIED ------------------------------------- #

  text_field(:user_id, id: 'usernameLarge')
  text_field(:password, id: 'passwordLarge')
  button(:login, id: 'btnLoginLarge')
  text_field(:security_answer, id: 'txtAnswer')
  # ul(:form_errors, xpath: '//*[@id="vsFormErrors"]/ul')
  # div(:error_div, id: 'divErrorMessage')

  def ready?
    user_id? && password? && login?
  end

  def populate
    super
    leave_page_using(:login, false)
  end
  #
  # def errors?
  #   form_errors? || error_div?
  # end

  def wait_for_ajax
    # The login page does not use jquery so turn this into a no op
  end

  def self.post(client, data = nil)
    data ||= data_for_this_page
    client.post_login_page(data)
  end
end
