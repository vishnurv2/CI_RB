...
CMD = "bundle exec cucumber #{opts[:cuke]} -p grid -p lambda_test -p should_work -p parallel_lb PLATFORM=#{opts[:platform]} BROWSER=#{opts[:browser]} BUILD=#{opts[:buildno]} VERSION=#{opts[:browserversion]} #{feature_folder}"
