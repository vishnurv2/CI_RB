...
opts = Optimist.options do
  ...
  opt :threads, "The number of thread to run", type: :string, default: '1'
...
# Run Cucumber
CMD = "bundle exec parallel_cucumber -n #{opts[:threads].to_i} -- #{opts[:cuke]} -p parallel -p should_work -- #{feature_folder}"
